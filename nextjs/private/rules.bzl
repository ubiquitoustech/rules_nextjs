...
nextjs_build = rule(
    ...
    attrs = {
        ...
        "_next": attr.label(
            ...
            default = Label("@npm//next/bin:next"),
            executable = True,
            cfg = "host",
        ),
...
def _nextjs_dev_impl(ctx):
    ...
    ctx.actions.expand_template(
        ...
        substitutions = {
            "TEMPLATED_main": _to_manifest_path(ctx, ctx.executable._next),
...
nextjs_dev = rule(
    ...
    attrs = {
        ...
        "_next": attr.label(
            doc = "An executable target that runs Nextjs",
            default = Label("@npm//next/bin:next"),
            executable = True,
            cfg = "host",
        ),
        ...
        "_launcher_template": attr.label(allow_single_file = True, default = Label("@ubiquitous_tech_rules_nextjs//nextjs/private:launcher_template.sh")),
    },
    outputs = {
        "script": "%{name}.sh",
    },
    doc = "Runs the dev server for nextjs",
)

def nextjs_devserver_macro(name, args = [], visibility = None, tags = [], testonly = 0, **kwargs):
    nextjs_dev(
